...
static irqreturn_t cdns_i2c_master_isr(void *ptr)
{
	...
	hold_quirk = (id->quirks & CDNS_I2C_BROKEN_HOLD_BIT);// && updatetx; Robsense modify: transfer 22bytes also will cause issue AR#61664*/
	...
	if (id->p_recv_buf &&
		...
		while (cdns_i2c_readreg(CDNS_I2C_SR_OFFSET) &
            ...
            {
                char val;
				/*Robsense modify: this should not happen, but in pxflow, it does happen sometime, so we have to do this to prevent buffer overflow*/
                val = cdns_i2c_readreg(CDNS_I2C_DATA_OFFSET);
            }
